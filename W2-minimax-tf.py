...
parser.add_argument('--ITERS', type=int, default=40000, help='number of iterations of training')
...
class ComputeOT:
	...
	def save_the_figure(self, iteration, X_plot, Y_plot, experiment_name, opt):
		...
		fig.savefig("figures/{0}/{1}_batch/{2}_layers/{3}_neurons/{4}_scale/{5}_variance/weightRegular_{6}/LR_{7}/trial_{8}/GT_{9}.png"
										.format(experiment_name, opt.BATCH_SIZE, opt.NUM_LAYERS, opt.NUM_NEURON,
																		 opt.SCALE, opt.VARIANCE,opt.LAMBDA, opt.LR ,opt.TRIAL,
																		  str(iteration)))
		np.save("figures/{0}/{1}_batch/{2}_layers/{3}_neurons/{4}_scale/{5}_variance/weightRegular_{6}/LR_{7}/trial_{8}/GT_{9}.npy"
										.format(experiment_name, opt.BATCH_SIZE, opt.NUM_LAYERS, opt.NUM_NEURON,
																		 opt.SCALE, opt.VARIANCE,opt.LAMBDA, opt.LR ,opt.TRIAL,
