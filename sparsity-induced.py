...
class ComputeOT:
	...
	def __init__(self, sess, input_dim, f_model, g_model,lr):
		...
		self.g_sparsity_penalty = self.lamda * self.S
	...
	def stvs(z, gamma):
		...
		sigma = tf.asinh(z / (2 * gamma))
		T_d = 1  # Adjust based on your dimensionality requirements
		return gamma * 0.5 * T_d * (sigma + 0.5 - 0.5 * tf.exp(-2 * sigma))
	...
	def save_the_figure(self, iteration, X_plot, Y_plot, experiment_name, opt):
		...
		fig.savefig("figures/{0}/{1}_batch/{2}_layers/{3}_neurons/{4}_scale/{5}_variance/weightRegular_{6}/LR_{7}/trial_{8}/OVK_0.01_{9}.png"
										.format(experiment_name, opt.BATCH_SIZE, opt.NUM_LAYERS, opt.NUM_NEURON,
																		 opt.SCALE, opt.VARIANCE,opt.LAMBDA, opt.LR ,opt.TRIAL,
																		  str(iteration)))
		np.save("figures/{0}/{1}_batch/{2}_layers/{3}_neurons/{4}_scale/{5}_variance/weightRegular_{6}/LR_{7}/trial_{8}/OVK_0.01_{9}.npy"
										.format(experiment_name, opt.BATCH_SIZE, opt.NUM_LAYERS, opt.NUM_NEURON,
																		 opt.SCALE, opt.VARIANCE,opt.LAMBDA, opt.LR ,opt.TRIAL,
...
def sample_data_gen(DATASET, BATCH_SIZE, SCALE , VARIANCE):
    ...
    # Generating points for X within the box defined by (0,0) and (3,2)
		while True:
			dataset_X = np.load(r'C:\Users\chenl\Desktop\SiCNN\2_dim_experiments\data\X.npy')
			yield dataset_X

	elif DATASET == 'Y':
		# Generating points for Y within two boxes
		while True:
		...
		while True:
			...
			# Second box for Y (4,0) to (5,2)
			for i in range(BATCH_SIZE // 2):
				x = np.random.uniform(800, 900)
				y = np.random.uniform(-50, 410)
				dataset_Y.append([x, y])
